...
-- 版本：v1.0
-- 日期：2026-05-16
-- 说明：初始化 admin/student 示例账号、5角色、8权限、系统参数
...
('admin', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iK6roS6zM59ycVcFQ4eFZ8B5WQ1a', '系统管理员', 'admin@seatflow.com', NULL, 'ADMIN'),
...
-- 6.1 为示例学生分配 student 角色（不授予管理端权限）
INSERT IGNORE INTO `t_user_role` (`user_id`, `role_id`)
SELECT u.id, r.id
FROM `t_user` u, `t_role` r
WHERE u.username IN ('student1', 'student2') AND r.code = 'student';
