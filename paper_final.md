...
**摘要**：本文以《软件过程管理》课程 Lab 项目——自习座位预约系统（SeatFlow）——为研究载体，系统记录并分析了在 OpenClaw + Claude 大语言模型驱动的 Vibe Coding 范式下，如何完成从需求理解、架构设计、多 Agent 初始化、代码生成到 CI/CD 交付的全流程实践。通过 49 次 Git 提交、112 个后端 Java 文件、24 个前端组件、47 个 REST API、12 个实测 Bug 和 10 类团队冲突问题的详细分析，本文提出了一套面向 AI 辅助编程的软件过程管理 SOP，归纳了 Vibe Coding 在上下文工程、需求拆解、隔离测试、多 Agent 编排等维度的利弊权衡，并对软件过程管理课程核心概念在 AI 时代的适应与挑战进行了批判性思考。
...
本文以《软件过程管理》课程 Lab 项目——自习座位预约系统（SeatFlow）——为具体载体，尝试回答上述问题。SeatFlow 是一个涵盖学生预约、签到、RBAC 权限管理与智能助手的 Web 全栈系统，由5名组员在 AI 辅助下协作开发，历时约一个月。本文不是泛泛而谈 AI 如何"提效"，而是从真实的提交记录、实测 Bug、团队冲突和流程决策出发，探讨 AI 时代软件过程管理的具体实践路径。

---

## 二、Lab 需求理解、任务拆分与我的工作

### 2.1 对 Lab 需求的理解

Lab 的核心需求是：**以高等学校日常学习生活为背景，通过信息技术手段实现一个自习室座位预约系统，提升各类自习室座位使用率**。

阅读需求文档后，我对项目的理解分三个层次：
...
**协作层**：5人团队分别负责不同模块，需要在接口边界、数据库 schema、分支策略上达成一致，避免并行开发产生冲突。

### 2.2 任务拆分与团队分工

Lab 采用用户故事（User Story）作为最小任务粒度，共识别17个用户故事，分为学生端（US-S01~S11）和管理端（US-A01~A06）：

**学生端任务分配**

| 编号 | 故事摘要 | 优先级 | 负责人 |
|---|---|---|---|
| US-S01 | 查看所有可用自习室及开放时间 | P0 | 组员C |
| US-S02 | 查看座位图并选择座位 | P0 | 组员A（我） |
| US-S03 | 按整点小时预约（最多4小时） | P0 | 组员C |
| US-S04 | 输入动态编码签到 | P0 | 组员D |
| US-S05 | 收到预约提醒（15min前推送） | P0 | 组员D |
| US-S06 | 超时未签到自动取消 | P0 | 组员D |
| US-S07 | 取消预约（含违约边界） | P0 | 组员C |
| US-S08 | 多条件搜索座位 | P0 | 组员E |
| US-S09 | 查看历史预约并再次预约 | P0 | 组员E |
| US-S10 | 查看违约记录 | P2 | 组员E |
| US-S11 | 智能助手自然语言交互 | P1 | 组员A（我） |
...
| US-A03 | 查看预约统计仪表盘 | P0 | 组员B |
| US-A04 | 代客预约/取消 | P2 | 组员B |
...
在正式开发前，我花了约一天时间搭建 OpenClaw 多 Agent 框架，编写以下配置文件：

- `SOUL.md`：定义 AI 的行为风格（不废话、直接动手、有异常立即报出）
- `USER.md`：记录项目技术约束（Java 路径、只用 pnpm、Podman 部署）
- `AGENTS.md`：定义 Agent 启动流程（先读上下文文件，再开始工作）
- `plan/plan.md`：作为所有 Code Agent 的"北极星"文档，统一接口规范和数据库设计

**架构设计**

我负责完成以下架构设计工作，并写入 `plan.md` 作为团队统一基准：
...
## 三、初始化阶段：基于 OpenClaw 的单 Agent 上下文搭建
...
## 七、Vibe Coding 过程总结与个人开发 SOP

### 7.1 Vibe Coding 的本质再理解
